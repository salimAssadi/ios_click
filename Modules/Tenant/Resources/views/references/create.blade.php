...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{ __('Create Reference') }}</li>
@endsection
...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('iso_dic.references.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="form-label" for="name_ar">{{ __('Reference Name (Arabic)') }} <span class="text-danger">*</span></label>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="form-label" for="name_en">{{ __('Reference Name (English)') }} <span class="text-danger">*</span></label>
                        ...
                        <div class="row mt-3">
                            <div class="col-12">
                                <label class="form-label">{{ __('ISO Systems') }} <span class="text-danger">*</span></label>
                                <div class="row">
                                    @foreach($isoSystems as $system)
                                        <div class="col-md-3 mb-2">
                                            <div class="form-check">
                                                <input type="checkbox" class="form-check-input @error('iso_systems') is-invalid @enderror" 
                                                    name="iso_systems[]" 
                                                    value="{{ $system->id }}" 
                                                    id="system_{{ $system->id }}"
                                                    {{ in_array($system->id, old('iso_systems', [])) ? 'checked' : '' }}>
                        ...
                        <div class="row mt-3">
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <small class="form-text text-muted">{{ __('Supported formats') }}: PDF, DOC, DOCX, XLS, XLSX. {{ __('Max size') }}: 10MB</small>
                                    @error('attachments.*')
                                        <span class="invalid-feedback" role="alert">{{ $message }}</span>
                                    @enderror
                                </div>
                        ...
                        <div class="row mt-4">
                            <div class="col-12">
                                ...
                                <a href="{{ route('iso_dic.references.index') }}" class="btn btn-secondary">{{ __('Cancel') }}</a>
                            </div>
                        </div>
                    </form>
                </div>
