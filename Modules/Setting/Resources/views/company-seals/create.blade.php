...
@section('page-title', __('Add New Seal'))
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">{{ __('Add New Seal') }}</li>
@endsection
@section('content')
    <div class="card mt-2">
        <div class="card-header row">
            <h3 class="card-title col">{{ __('Add New Seal') }}</h3>
        ...
        <div class="card-body">
            <form action="{{ route('tenant.setting.company-seals.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    <label for="seal_file">{{ __('Seal File') }} <span class="text-danger">*</span></label>
                    <div class="custom-file">
                        <input type="file" name="seal_file" id="seal_file" class="custom-file-input @error('seal_file') is-invalid @enderror" required>
                        <label class="custom-file-label" for="seal_file">{{ __('Choose file') }}</label>
                        @error('seal_file')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    <small class="form-text text-muted">{{ __('Accepted file types: jpeg, png, jpg, gif, svg') }}</small>
                </div>
