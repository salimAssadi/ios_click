...
<form method="post" action="{{ route('iso_dic.setting.theme.settings') }}">
    {{ csrf_field() }}
...
<script>
     var theme = 'default'; // Define your theme here
  $.ajax({
      url: "{{ route('iso_dic.tree.data')}}",
      method: "GET",
      success: function(data) {
          if ($('#iso-tree').length) {
              $('#iso-tree').jstree({
                  core: {
                      themes: {
                          name: theme // Ensure 'theme' is defined elsewhere
                      },
                      check_callback: true,
                      data: data
                  },
