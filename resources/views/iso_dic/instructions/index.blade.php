@extends('layouts.admin-app')
@section('page-title')
    {{ __('Instructions') }}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('iso_dic.dashboard') }}">{{ __('Dashboard') }}</a></li>
    <li class="breadcrumb-item">{{ __('Instructions') }}</li>
@endsection



@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-header row">
                    <div class="col">
                        <h5>{{ __('Instructions') }}</h5>
                    </div>
                    <div class="col-auto">
                        <a href="{{ route('iso_dic.instructions.create') }}" class="btn btn-sm btn-secondary" data-bs-toggle="tooltip"
                            title="{{ __('Create') }}">
                            <i class="ti ti-plus"></i>
                        </a>
                    </div>
                </div>
                <div class="card-body table-border-style">
                    <div class="table-responsive">
                        <table class="table datatable">
                            <thead>
                                <tr>
                                    <th>{{ __('Name') }}</th>
                                    <th>{{ __('Related Procedures') }}</th>
                                    <th>{{ __('Attachments') }}</th>
                                    <th>{{ __('Status') }}</th>
                                    <th width="200px">{{ __('Action') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($instructions as $instruction)
                                    <tr>
                                        <td>
                                            {{ app()->getLocale() == 'ar' ? $instruction->name_ar : $instruction->name_en }}
                                        </td>
                                        <td>
                                            <div class="d-flex flex-wrap gap-1">
                                                @foreach($instruction->procedures as $procedure)
                                                    <span class="badge bg-primary">
                                                        {{ $procedure->procedure_name }}
                                                    </span>
                                                @endforeach
                                            </div>
                                        </td>
                                        <td>
                                            @foreach($instruction->attachments as $attachment)
                                                <div class="mb-1">
                                                    <a href="{{ route('iso_dic.instructions.attachments.download', $attachment->id) }}" 
                                                       class="text-primary">
                                                        {{ $attachment->original_name }}
                                                    </a>
                                                </div>
                                            @endforeach
                                        </td>
                                        <td>
                                            @if($instruction->is_published)
                                                <span class="badge bg-success">{{ __('Published') }}</span>
                                            @else
                                                <span class="badge bg-warning">{{ __('Unpublished') }}</span>
                                            @endif
                                        </td>
                                        <td class="Action d-flex gap-2">
                                            <div class="action-btn">
                                                <a href="{{ route('iso_dic.instructions.edit', $instruction->id) }}"
                                                    class="mx-3 btn btn-sm align-items-center btn-primary"
                                                    data-bs-toggle="tooltip" title="{{ __('Edit') }}">
                                                    <i class="ti ti-pencil text-white"></i>
                                                </a>
                                            </div>
                                            <div class="action-btn">
                                                {!! Form::open(['method' => 'DELETE', 'route' => ['iso_dic.instructions.destroy', $instruction->id], 'class' => 'd-inline']) !!}
                                                <a href="#" class="mx-3 btn btn-sm align-items-center btn-danger show_confirm"
                                                    data-bs-toggle="tooltip" title="{{ __('Delete') }}">
                                                    <i class="ti ti-trash text-white"></i>
                                                </a>
                                                {!! Form::close() !!}
                                            </div>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center">{{ __('No instructions found') }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
